            ...
            <div class="panel-body">
              <div class="row">
               ...
               <div class="col-lg-2 col-lg-2">
                  ...
                  <input type="number" onkeyup="item_total ('input_item<?=$item_row['item_id'];?>', '<?=$item_row['price'];?>', 'total_response<?=$item_row['item_id'];?>', 'total-items<?=$item_row['item_id'];?>', 'btn-response<?=$item_row['item_id'];?>', 'text-response<?=$item_row['item_id'];?>')" id="input_item<?=$item_row['item_id'];?>" class="form-control input-lg" name="quantity"  min="1" max="<?=$item_row['quantity'];?>" value="1">
                  <input type="hidden" value="<?=$item_row['quantity'];?>" id="total-items<?=$item_row['item_id'];?>">
                  <p id="text-response<?=$item_row['item_id'];?>" style="display: none; color: #ff0000;">Enter items from 1 to <?=$item_row['quantity'];?> only.</p>
               </div>
               <div class="col-lg-2 col-lg-2">
               ...
               <div class="col-lg-2 col-lg-2">
                  <button type="submit" style="margin-top: 35px;" class="btn btn-success btn-lg btn-block" onclick="form_process ('savecart-form<?=$item_row['item_id'];?>', '../functions/save-cart.php', 'savecart-response<?=$item_row['item_id'];?>')" id="btn-response<?=$item_row['item_id'];?>">Add</button>
               </div> 
            </div>
            </div>
          </div>
